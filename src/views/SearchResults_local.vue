<template>  
    ...
    <div v-if="Object.keys(searchResults).length > 0" class="search-results">  
        <ul>  
            <li v-for="( item, index ) in searchResults.menu_data" :key="index">  
              ID: {{ item.id }}, Name: {{ item.name }}, Distance: {{ item.distance }} 
            </li>  
        </ul>  
    </div>
    <div v-if="Object.keys(searchResults).length > 0" class="search-results">  
        <ul>  
          <li v-for="(item, index) in searchResults.result_ids" :key="index">  
  ...
  export default {  
    ...
    fetchResults() {  
      if (this.searchText.trim() !== '') {  
        axios.get(`/local-api/search_text?input_text=${encodeURIComponent(this.searchText)}&k=10`)  
          .then(response => {    
